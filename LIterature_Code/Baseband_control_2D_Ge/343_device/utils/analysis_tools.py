...
#     # for long measurements with decay use:
#     amplitude_guess = 2 * np.std(data) * 1.5

#     # for short measurements without strong decay use:
...
#     try:
#         best_fit_params, cov_matrix = curve_fit(Ramsey, x, data,
#                                                 p0=initial_guess,
#                                                 # bounds=([0, 0.001, -np.pi, 100, 0], [1, 0.04, np.pi, 100e3, 1.0])
#                                                 bounds=([amplitude_guess - 0.10, freq_guess - 0.01, -np.pi, 100, 0],
#                                                         [amplitude_guess + 0.10, freq_guess + 0.01, np.pi, 100e3, 1.0])
#                                                 )

#         # Display the best-fit parameters
#         print("Best-fit parameters:")
#         print("A:", best_fit_params[0])
#         print("Frequency:", best_fit_params[1])
#         print("Phase:", best_fit_params[2])
#         print("Decay Time (tau):", best_fit_params[3])
#         print("Constant Offset (C):", best_fit_params[4])
...
def fit_Ramsey_data(x, data, freq_guess, short_measurement=False, tau_guess=10e3):
    ...
    try:
        ...
        param_names = ["Amplitude (A)", "Frequency", "Phase",
                       "Decay Time (tau)", "Constant Offset (C)"]
        for i, (param, error) in enumerate(zip(best_fit_params, perr)):
            print(f"{param_names[i]}: {param:.3f} \u00B1 {error:.3f}")
