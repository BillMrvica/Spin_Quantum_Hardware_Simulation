...
from qutip import *
# try:
#     from qutip.interpolate import Cubic_Spline # it only exists in the QuTip 4, and it's removed in QuTip 5
#     print("qutip.interpolate.Cubic_Spline is available:", Cubic_Spline)
# except Exception:
#     from scipy.interpolate import CubicSpline as Cubic_Spline
#     print("Using scipy.interpolate.CubicSpline as fallback:", Cubic_Spline)
import time
...
for print_residual in [True, False]:
    ...
    for it in range(len(waittimes)):
        ...
        # Using QuTip 4, the following works:    
        S_e = Cubic_Spline(ts[0], ts[-1], es)

        # Using QuTip 5, the following works:
        # S_e = (ts, es)
        ...
        if print_residual:
            ...
            r = [len(ts)-1]
        for idx in r:        
            evals, ekets = (H[0] + H[1][0]*H[1][1](ts[idx])  ).eigenstates()
        ...
        H_r = [2*np.pi*H_q3 , 
             [ 2*np.pi*(-H_q3 + H_q2), S_sinetheta2],   
              ]
        
        
        evals, ekets = (H_r[0] + H_r[1][0]*H_r[1][1](ts[0])  ).eigenstates()
        psi0 = ekets[0] 
        ...
        if print_residual:
            ...
            r = [len(ts)-1]
        for idx in r:
            evals, ekets = (H_r[0] + H_r[1][0]*H_r[1][1](ts[idx])  ).eigenstates()
        ...
        H_q2_simple = (H_r[0] + H_r[1][0]*H_r[1][1](ts[0])  ) /(2*np.pi)
        H_q3_simple = (H_r[0] + H_r[1][0]*H_r[1][1](ts[ ramptime *M_pts])  ) /(2*np.pi)
...
import pickle
pickle.dump(results , open('FigSupp5_qutip_simulation_20251017.pickle', 'wb'))
