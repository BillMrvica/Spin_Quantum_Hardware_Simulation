# this file is modified from Supp/uneuql_time/bloch_slider_without_qutip_v3_q1x90.py
...
DEG = np.pi/180
fq2 = 3.38324710e-02  
fq3     = 7.15275643e-02    # qubit  frequency
...
tlist = np.arange(0, wait_q4_1  , dt)
...
tlist = np.arange( wait_q4_1 , wait_q4_1 + wait_q1  , dt)
for j in range(len(tlist)):
    output = dU2 @ output
    outputs += [output]
tlists = np.hstack((tlists,tlist)) 
...
tlist = np.arange( wait_q4_1+wait_q1, wait_q4_1+wait_q1+wait_q4_2  , dt)
for j in range(len(tlist)):
    output = dU3 @ output
    outputs += [output    ]
tlists = np.hstack((tlists,tlist))  
...
tlist = np.arange( wait_q4_1+wait_q1+wait_q4_2 ,   wait_q4_1+wait_q1+wait_q4_2+add_wait_q1  , dt)
for j in range(len(tlist)):
    output = dU2 @ output
    outputs += [output]
tlists = np.hstack((tlists,tlist)) 
...
sphere_interact = None

if sphere_interact==None:

    # fig, axs = plt.subplots( figsize=(7,9))
    pltname = 'Interactive plot with slider'
    fig = plt.figure( pltname, figsize = (7,9))
    sphere_interact=Bloch(fig)

else:
    sphere_interact.clear()    
colorlist = list(plt.cm.Oranges(np.linspace(0.9,0,301)))
sphere_interact.vector_color = ['g', colorlist[0]] 
sphere_interact.point_color = colorlist
sphere_interact.add_vectors(hlists_plot[0])
sphere_interact.add_states( state2vec(psi00)  )
sphere_interact.render()
...
def update(val):
    ...
    sphere_interact.add_vectors(hlists_plot[idx])
    sphere_interact.add_states(  state2vec(outputs_plot[idx])  )
    
    if idx > 0:
        k = max(idx-300, 0)
        vx, vy, vz = expect(sx, outputs_plot[idx:k:-1]), expect(sy, outputs_plot[idx:k:-1]), expect(sz, outputs_plot[idx:k:-1]) 
        sphere_interact.add_points([vx,vy,vz], 'm')
        # print(vx)
...
colorlist = ['C0',]*(switch_pts[0]+1) \
            + ['C1',]*(switch_pts[1]-switch_pts[0]) \
            + ['C2',]*(switch_pts[2]-switch_pts[1]) \
                + ['C3',]*(len(temp_var)-switch_pts[2]-1) 
sphere_static.point_color = colorlist
sphere_static.add_points([vx,vy,vz], 'm')
sphere_static.render()


# filename = 'FigureSupp7b' # 'Fig_Supp_q1x90_equal'
...
plt.plot(  t + t_x , y_x  , CQ1 )
t = ( t + t_x)[-1]     


plt.title('Qubit A equal time gate voltage pulses')
plt.xlabel('time (ns)')
