...
DEG = np.pi/180
fq2 = 0.070926  
fq3     = 0.06203767    # qubit frequency
...
tlist = np.arange( wait_q3+wait_q2 , wait_q3+wait_q2+wait_q3 , dt)
for j in range(len(tlist)):
    output = dU3 @ output
    outputs += [output    ]
tlists = np.hstack((tlists,tlist))  
...
tlist = np.arange( wait_q3+wait_q2+wait_q3, wait_q3+wait_q2+wait_q3+2.288 , dt)
for j in range(len(tlist)):
    output = dU2 @ output
    outputs += [output]
tlists = np.hstack((tlists,tlist)) 
...
plt.title('Qubit B equal time gate voltage pulses')
plt.xlabel('time (ns)')
