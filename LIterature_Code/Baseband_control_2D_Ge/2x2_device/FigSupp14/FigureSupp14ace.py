...
# plt.subplots_adjust(bottom=0.15, top=0.9, left=0.05, right=0.95, wspace=0.3)
# plt.suptitle('data files from' )

ax = ax13

ax.grid(linewidth=0.5)  
mask = vpB12<-10
# ax.scatter(vpB12[mask], (dnup_upup - dndn_updn)[mask]   , c='b', s=5,  label=r'$\Delta fq1$')
# plt.scatter(vpB12[mask], (updn_upup - dndn_dnup)[mask]   , c='b', label=r'$\Delta fq2$')
# ax.scatter(vpB12_long, (dnup_upup_long - dndn_updn_long)    , c='b', s=5,  label=r'$\Delta fq1$')
...
ax.plot(vpB12, dndn_updn, marker='v', c=CQ1, markersize=MRSIZE, label='fq1 (q2 down)', linestyle='none')
ax.plot(vpB12, dnup_upup, marker='^', c=CQ1, markersize=MRSIZE, label='fq1 (q2 up)', linestyle='none')



ax.plot(vpB12, dndn_dnup, marker='v', c=CQ2, markersize=MRSIZE, label='fq2 (q1 down)', linestyle='none')
ax.plot(vpB12, updn_upup, marker='^', c=CQ2, markersize=MRSIZE, label='fq2 (q1 up)', linestyle='none')

# ax.scatter(vpB12_long, (dnup_upup_long )   , c='g', label=r'$\Delta fq1$')
# ax.scatter(vpB12_long, ( dndn_updn_long)   , c='g', label=r'$\Delta fq1$')
...
# plt.plot( xxx, twoQfreq(xxx, tso_c, tso_theta)[0] , c=CQ1 )
# mx = vpB12< -40
mx = vpB12< 60

idxsort = np.argsort(vpB12[mx])
xxx0 = vpB12[mx][idxsort]
xdata = np.hstack([xxx0, xxx0, xxx0, xxx0] )
zdata = np.hstack([dndn_updn[mx][idxsort], dnup_upup[mx][idxsort], dndn_dnup[mx][idxsort], updn_upup[mx][idxsort]] )
...
fq1_avg = (dndn_updn+dnup_upup)/2
mask1 = vpB12>-10
L1 = np.polyfit( vpB12[mask1], fq1_avg[mask1],  deg=1)

fq2_avg = (dndn_dnup+updn_upup)/2
mask2 = vpB12>-10
L2 = np.polyfit( vpB12[mask2], fq2_avg[mask2],  deg=1)

L1_0, L1_1, L2_0, L2_1 = *L1, *L2
p0 = [ 1.2, 5.6 ,  0.24,  L1_0, L1_1, L2_0, L2_1 ]
p1_2Qfit, pcov = curve_fit(twoQfreq, xdata, zdata, p0=p0, )
# output: p1_2Qfit = np.array([ 1.26609665e+00,  5.60351262e-01,  3.04382405e-01, -1.05262210e-01,
...
zdata_div = {}
for i in range(4):
    zdata_div[str(i)] = twoQfreq(xdata, *p1_2Qfit) [i*Ndiv:i*Ndiv+Ndiv]

for i in range(4):
    ax.plot(xxx0, zdata_div[str(i)] ,'k' , linewidth=0.5)

##########################


ax.set_xticks([-90, -20, 50 ])
ax.set_yticks([40, 80, 120, 160])  



ax = ax11 
xxx = np.linspace(-100, 0, 201)
evs = twoQlevels(xxx, *p1_2Qfit)
ax.plot(xxx, evs[0,:],  c='C0',  label='g', linestyle='-', linewidth=1)
ax.plot(xxx, evs[1,:],  c='C1',  label='e1', linestyle='-', linewidth=1)
ax.plot(xxx, evs[2,:],  c='C2',  label='e2', linestyle='-', linewidth=1)
ax.plot(xxx, evs[3,:],  c='C3',  label='e3', linestyle='-', linewidth=1)
...
ax = ax33
ax.scatter(vpB12, (dnup_upup - dndn_updn)   , s=SCSIZE, c=CQ1, label=r'$\Delta fq1$')
ax.scatter(vpB12, (updn_upup - dndn_dnup)   , s=SCSIZE, c=CQ2, label=r'$\Delta fq2$')
ax.plot(vpB12, 0.24*np.exp(-0.059*(vpB12-10) )  , 'C2', linewidth=0.5  )
# ax.xlabel('_1100_cz_vpB12 (mV)')
...
ax = ax53
xxx = np.linspace( -90, 80,2)
ax.plot(xxx, 0.24*np.exp(-0.059*(xxx-10) )  , 'C2', linewidth=0.5  )
...
for i in range(len(vpB12[mask])):
    ax.errorbar(vpB12[mask][i], (dnup_upup - dndn_updn)[mask][i],
                 ...
                 fmt ='o', markersize=MRSIZE, linestyle='', linewidth=0.5,  label=r'$\Delta fq1$')
# plt.scatter(vpB12[mask], (updn_upup - dndn_dnup)[mask]   , c='b', label=r'$\Delta fq2$')
...
for i in [0]:
    ax.errorbar( vpB12_hahn[i], df_hahn[i],
                 yerr = df_hahn_err[i], c='r',
                 fmt ='^', markersize=MRSIZE, linestyle='', linewidth=0.5 )




ax.set_yscale('log')    
ax.set_ylim(0.005, 80)
...
ax.plot(example_plots['fq1_q2dn']['xdata'], example_plots['fq1_q2dn']['zdata'], marker='v', c=CQ1, markersize=MRSIZE, label='fq1 (q2 down)', linestyle='')
xxx = example_plots['fq1_q2dn']['xdata']
xxx = np.linspace(min(xxx),max(xxx),len(xxx)*5)
p1 = example_plots['fq1_q2dn']['p1']
func = example_plots['fq1_q2dn']['func']
ax.plot(xxx, func(xxx, *p1), marker='', c='k',  label='fq1 (q2 up)', linestyle='-', linewidth=0.5)

ax.plot(example_plots['fq1_q2up']['xdata'], example_plots['fq1_q2up']['zdata'], marker='^', c=CQ1, markersize=MRSIZE, label='fq1 (q2 up)', linestyle='')
xxx = example_plots['fq1_q2up']['xdata']
xxx = np.linspace(min(xxx),max(xxx),len(xxx)*5)

p1 = example_plots['fq1_q2up']['p1']
func = example_plots['fq1_q2up']['func']
ax.plot(xxx, func(xxx, *p1), marker='', c='k',  label='fq1 (q2 up)', linestyle='--', linewidth=0.5)
...
# filename = 'FigureSupp14ace' # 'fq1fq2_exchange_on'
