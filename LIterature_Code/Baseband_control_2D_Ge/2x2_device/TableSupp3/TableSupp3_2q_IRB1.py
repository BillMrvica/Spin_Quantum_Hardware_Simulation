# this file is modified from Supp\2q_RB_all\2qRB_reorder_2023-08-22_07-35-53_CI_supp.py
...
fdir = ''
all_sequences_data = pickle.load(  open(fdir + '2qRB_reorder_2023-08-22_07-35-53.pickle',  'rb')      )
...
# reference sequence decay rate  = 0.9792
# interleaved sequence decay rate = 0.9688
# 2Q Clifford infidelity = 0.01559
# interleaved CZ gate infidelity = 0.007941
# Infinite reference sequence will decay to 0.2954
# Infinite interleaved sequence will decay to 0.2844
# #######
# If not enforce alpha=1 but a free fitting parameter:
# reference sequence alpha  = 0.8619
# interleaved sequence alpha = 0.8672
# reference sequence decay rate  = 0.966
# interleaved sequence decay rate = 0.9505
# 2Q Clifford infidelity = 0.02551
# interleaved CZ gate infidelity = 0.01199
# Infinite reference sequence will decay to 0.2708
# Infinite interleaved sequence will decay to 0.2721
...
for i_interleaved in ['RB', 'IRB',  ]:
    ...
    for sequence_idx in range(1,num_sequences):
        ...
        for ii in range(L):
            ...
            temp_list += [ np.sum( np.logical_and( temp_psb14 < vth0, temp_psb23 > vth1 ) )/len(temp_psb14) ] 
...
results = pickle.load( open('2qRB_reorder_2023-08-22_07-35-53_bootstrap_resample.pickle', 'rb') )['results']
...
print('')
print('original data: 2Q Clifford avg infidelity = 0.01559')
print(f'bootstrap resampling average infidelity = {np.average(avg_err_ref_sort):.5g}', )
print('5% = ', avg_err_ref_sort[int(0.05*len(avg_err_ref_list))] )
print('95% = ', avg_err_ref_sort[int(0.95*len(avg_err_ref_list))] )
print(f'-> 2Q Clifford avg infidelity = 0.01559 +- {abs(avg_err_ref_sort[int(0.95*len(avg_err_ref_list))]-avg_err_ref_sort[int(0.05*len(avg_err_ref_list))])/2:.4g}, with 95% confidence interval')


print('original data: interleaved gate avg infidelity = 0.007941')
print(f'bootstrap resampling average infidelity = {np.average(avg_interleaved_gate_infidelity_list):.5g}', )
print('5% = ', avg_interleaved_gate_infidelity_list[int(0.05*len(avg_err_ref_list))] )
print('95% = ', avg_interleaved_gate_infidelity_list[int(0.95*len(avg_err_ref_list))] )
print(f'-> interleaved gate avg infidelity = 0.007941 +- {abs(avg_interleaved_gate_infidelity_list[int(0.95*len(avg_err_ref_list))]-avg_interleaved_gate_infidelity_list[int(0.05*len(avg_err_ref_list))])/2:.4g}, with 95% confidence interval')

# result of N_resample = 10000: 
# original data: 2Q Clifford avg infidelity = 0.01559
# bootstrap resampling average infidelity = 0.015572
# 5% =  0.01492438419905695
# 95% =  0.016235554423008197
# -> 2Q Clifford avg infidelity = 0.01559 +- 0.0006556, with 95% confidence interval
# original data: interleaved gate avg infidelity = 0.007941
# bootstrap resampling average infidelity = 0.0079679
# 5% =  0.006872325231595644
# 95% =  0.00907028333836049
# -> interleaved gate avg infidelity = 0.007941 +- 0.001099, with 95% confidence interval
...
print('')
print( 'ref sequence alpha = 0.8619 +- ' + f'{(tempvar95-tempvar05)/2:.5g}'  )
...
print( 'interleaved sequence alpha = 0.8672 +- ' + f'{(tempvar95-tempvar05)/2:.5g}'  )
...
print( 'reference sequence alpha  = 0.8619'  )
print('original data with free alpha: 2Q Clifford avg infidelity = 0.02551')
...
print(f'-> 2Q Clifford avg infidelity = 0.02551 +- {abs(val_ref_sort[int(0.95*len(avg_err_ref_list))]-val_ref_sort[int(0.05*len(avg_err_ref_list))])/2:.4g}, with 95% confidence interval')

print( 'interleaved sequence alpha = 0.8672' )
print('original data with free alpha: interleaved gate avg infidelity = 0.01199')
...
print(f'-> interleaved gate avg infidelity = 0.01199 +- {abs(val_interleaved_sort[int(0.95*len(avg_err_ref_list))]-val_interleaved_sort[int(0.05*len(avg_err_ref_list))])/2:.4g}, with 95% confidence interval')



# reference sequence alpha  = 0.8619
# original data with free alpha: 2Q Clifford avg infidelity = 0.02551
# bootstrap resampling average infidelity = 0.025631
# 5% =  0.02183968364946251
# 95% =  0.029797259804086124
# -> 2Q Clifford avg infidelity = 0.02551 +- 0.003979, with 95% confidence interval
# interleaved sequence alpha = 0.8672
# original data with free alpha: interleaved gate avg infidelity = 0.01199
# bootstrap resampling average infidelity = 0.011865
# 5% =  0.00506748814211197
# 95% =  0.018629118981919407
# -> interleaved gate avg infidelity = 0.01199 +- 0.006781, with 95% confidence interval
