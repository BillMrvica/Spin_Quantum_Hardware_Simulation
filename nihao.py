...
# ===================================================================
#                      New code to connect device and pads
# ===================================================================
def route_connections(main_cell, device_instance, device_conns, pad_conns, all_pads, trace_width=2):
    """
    Routes connections between the device instance and the pad frame using simple Manhattan routing.
    """
    LAYER_TRACE = 1  # Use the same layer as pad traces
    device_origin = device_instance.origin
    
    # Create a dictionary for quick lookup of pad info by label
    all_pads_dict = {p['label']: p for p in all_pads}

    for label, dev_pt in device_conns.items():
        if label in pad_conns and label in all_pads_dict:
            pad_info = all_pads_dict[label]
            pad_pt = pad_conns[label]
            pad_edge = pad_info.get('target_edge')

            if not pad_edge:
                print(f"Warning: No target_edge found for pad '{label}'. Skipping routing.")
                continue

            # Transform device connection point to main cell coordinates
            dev_world_pt = (dev_pt[0] + device_origin[0], dev_pt[1] + device_origin[1])

            # Create a path object
            path = gdstk.FlexPath(dev_world_pt, trace_width, layer=LAYER_TRACE, ends="flush", joins="miter")
            
            # Simple Manhattan routing logic: route along the axis of the pad edge first
            if pad_edge in ['top', 'bottom']:
                path.segment(pad_pt, axis="y")
            else:  # 'left', 'right'
                path.segment(pad_pt, axis="x")

            main_cell.add(path)

def plot_final_chip(cell, title="Fully Routed 6QD Chip"):
    """
    Custom plotting function to visualize the final integrated chip with appropriate layers.
    """
    fig, ax = plt.subplots(figsize=(20, 20))
    ax.set_title(title, fontsize=18)
    ax.set_aspect('equal')
    
    # Define colors and properties for all layers used in the final layout
    layer_config = {
        0: {'color': '#87CEEB', 'alpha': 0.6, 'label': 'Device S/D (L0)'},
        1: {'color': '#A9A9A9', 'alpha': 0.8, 'label': 'Traces/Pads/SG (L1)'},
        2: {'color': '#FF1493', 'alpha': 0.9, 'label': 'Device BG (L2)'},
        3: {'color': '#8A2BE2', 'alpha': 0.9, 'label': 'Device PG (L3)'},
        10: {'color': 'lightblue', 'alpha': 0.3, 'label': 'Active Area (L10)'},
        11: {'color': 'lightgrey', 'alpha': 0.2, 'label': 'SiO2 Opening (L11)'},
    }

    drawn_labels = set()
    all_polygons = cell.get_polygons(by_spec=True)

    # Sort layers to control drawing order (e.g., draw active area first)
    sorted_layers = sorted(all_polygons.keys(), key=lambda k: k[0])

    for layer, datatype in sorted_layers:
        if layer in layer_config:
            cfg = layer_config[layer]
            label = cfg['label']
            polygons = all_polygons[(layer, datatype)]
            for gds_poly in polygons:
                pts = gds_poly.points
                if pts.ndim != 2 or len(pts) < 3: continue
                
                current_label = label if label not in drawn_labels else None
                poly = MplPolygon(pts, closed=True, facecolor=cfg['color'], edgecolor='black', 
...
if __name__ == "__main__":
    # 1. Create the pad frame layout
    pad_lib, pad_cell, pad_conns, active_center, all_pads = create_rect_wire_layout()

    # 2. Create the 6QD device layout
    device_lib, device_cell, device_conns = create_6qd_layout_with_labels()

    # 3. Create the final integrated chip cell in a new library
    chip_lib = gdstk.Library()
    chip_cell = chip_lib.new_cell('ROUTED_6QD_CHIP')

    # 4. Add the pad frame to the chip cell
    pad_ref = gdstk.Reference(pad_cell)
    chip_cell.add(pad_ref)

    # 5. Calculate device placement and add it to the chip cell
    # The device layout is not centered at (0,0), so we find its bounding box center
    # to align it with the active_center of the pad frame.
    device_bbox = device_cell.bounding_box()
    device_center_offset_x = (device_bbox[0][0] + device_bbox[1][0]) / 2
    device_center_offset_y = (device_bbox[0][1] + device_bbox[1][1]) / 2
    
    device_origin = (active_center[0] - device_center_offset_x, active_center[1] - device_center_offset_y)
    
    device_ref = gdstk.Reference(device_cell, origin=device_origin)
    chip_cell.add(device_ref)

    # 6. Route the connections between the device and the pads
    route_connections(chip_cell, device_ref, device_conns, pad_conns, all_pads, trace_width=4)

    # 7. Save the final layout to a GDS file and plot the result
    output_gds_file = 'final_routed_6qd_chip.gds'
    chip_lib.write_gds(output_gds_file)
    print(f"Successfully saved the final routed layout to '{output_gds_file}'")
    
    plot_final_chip(chip_cell, title="Fully Routed 6QD Chip")
