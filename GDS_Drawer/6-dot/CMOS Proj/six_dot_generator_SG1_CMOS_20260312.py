...
Modify for the CMOS project. Only SET1_D/SET2_D/SET_S are modified to have annealing S/D layer (L7) in addition to accumulation S/D layer (L0).
"""
...
LAYER_SD_ACCUM = 5       # Accumulation Source/Drain - 累积源漏层（浅蓝色）
...
def plot_gds(cell, title="量子点器件布局", show_text_labels=True):
    """
    可视化GDS布局（包含多层多边形 + 可选的 GDS Label 文本）
    ...
    show_text_labels : bool
        是否在图中绘制 gdstk.Label（layer=LAYER_LABEL）的文字
    """
    ...
    layer_config = {
        LAYER_SD_ANNEAL: {'color': '#FF8C00', 'alpha': 0.85, 'label': 'Annealing S/D (L1) - 退火源漏', 'zorder': 1},
        LAYER_SG: {'color': '#D3D3D3', 'alpha': 0.45, 'label': 'SG (L2) - 屏蔽门', 'hatch': '///', 'zorder': 2},
        LAYER_BG: {'color': '#FF69B4', 'alpha': 0.90, 'label': 'BG (L3) - 势垒门', 'zorder': 3},
        # 第4层：PG + Accum SD，颜色区分
        LAYER_PG: {'color': '#7B68EE', 'alpha': 0.90, 'label': 'PG (L4) - 柱栅', 'zorder': 4},
        LAYER_SD_ACCUM: {'color': '#4DB6FF', 'alpha': 0.65, 'label': 'Accumulation S/D (L4) - 累积源漏', 'zorder': 4},
    }
    ...
    # 叠加绘制文字标签（GDS Label）
    if show_text_labels:
        # gdstk.Cell.labels 是该 cell 直接持有的标签列表；此脚本的 _add_label() 也是直接 add 到 cell。
        for lbl in getattr(cell, "labels", []):
            try:
                if getattr(lbl, "layer", None) != LAYER_LABEL:
                    continue
                x, y = lbl.origin
                ax.text(
                    x, y, lbl.text,
...
def create_6qd_device_core(
    ...
    # SG标签
    _add_label(cell, connection_points, "SG1", (sg2xs, (ysgtt2 + ysgtt) / 2))
    _add_label(cell, connection_points, "SG2", (sg2xe, (ysgmb + ysgmt) / 2))
    ...
    sd_shapes.append(create_rounded_rect(golx, ycb - sd_height / 2,
                                          ...
                                          [False, True, True, False], layer=LAYER_SD_ANNEAL))
    ...
    _add_label(cell, connection_points, "QD_D", (golx, ycb))
    ...
    sd_shapes.append(create_rounded_rect(brsix, ycb - sd_height / 2,
                                          ...
                                          [True, False, False, True], layer=LAYER_SD_ANNEAL))
    ...
    _add_label(cell, connection_points, "QD_S", (gorx, ycb))
...
if __name__ == "__main__":
    ...
    # 保存GDS
    print("\n保存GDS文件...")
    save_gds(lib, os.path.join(output_dir, "6qd_device_core_with_SET_anneal_20260312.gds"))
