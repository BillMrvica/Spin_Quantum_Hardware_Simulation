...
class QuantumDotPadGenerator18DotV3:
    def __init__(self):
    ...
    def _plot_gds(self, cell, title="18-dot 量子点器件布局 (V3)", show_plot=True):
        if not show_plot:
            return
    ...
    def _create_18qd_layout_with_labels(self, **kwargs):
        ...
        # sd_shps.append(gdstk.rectangle((msdcx_s1d - sd_lead_width / 2, yct), (msdcx_s1d + sd_lead_width / 2, mlye), layer=LAYER_SD))
        ...
        # sd_shps.append(gdstk.rectangle((msdcx_s6d - sd_lead_width / 2, yct), (msdcx_s6d + sd_lead_width / 2, mlye), layer=LAYER_SD))
        ...
        for i in range(1, 6):
            ...
            msdcx = (smxm+smxa)/2
            sd_shps.append(cr(smxm, yct-sd_height/2, smxa, yct+sd_height/2, [True,True,True,True]))
        ...
        ysgtb=yct+gate_half_height+gap_gate_outer_sg;ysgtt=ysgtb+sg_top_thick;
        neck_half_width = 0.14
        ...
        ysgbt=ycb-gate_half_height-gap_gate_outer_sg;ysgbb=ysgbt-sg_bottom_thick;
    ...
    def _create_rect_wire_layout(self, **kwargs):
        ...
        # Support independent width and height for the first active region, 
        # falling back to active_size if not provided.
        ...
        def ac(pads,edge):
            ...
            if edge in['top','bottom']:
                ...
                for i,lbl in enumerate(pads):
                    x=sx+i*(pad_width+pad_spacing);cx,cy=x+pad_width/2,y+pad_height/2
                ...
                # left/right pads are rotated: rect width = pad_height, rect height = pad_width
                # The vertical stepping must use pad_width so that the edge-to-edge gap equals pad_spacing.
        ...
        cx=layout_width/2;cy=layout_height/2;acenter=(cx,cy)
        ax1=cx-active_width/2;ax2=cx+active_width/2;ay1=cy-active_height/2;ay2=cy+active_height/2
        pbe={'top':[],'bottom':[],'left':[],'right':[]};[pbe[p['edge']].append(p)for p in all_pads]
        ...
        def at_inner(pads, edge, pitch, target_y_top, target_y_bot, target_x_left, target_x_right, suffix):
            n = len(pads)
            if n == 0: return
            ind = np.arange(n) - (n - 1) / 2; offs = ind * pitch
            if edge == 'top': [p.update({f'target_{suffix}': (cx + offs[i], target_y_top)}) for i, p in enumerate(pads)]
            elif edge == 'bottom': [p.update({f'target_{suffix}': (cx + offs[i], target_y_bot)}) for i, p in enumerate(pads)]
            elif edge == 'left': [p.update({f'target_{suffix}': (target_x_left, cy + offs[-(i + 1)])}) for i, p in enumerate(pads)]
            else: [p.update({f'target_{suffix}': (target_x_right, cy + offs[-(i + 1)])}) for i, p in enumerate(pads)]
        ...
        LS=30;LP=31;LT=31;LTXT=40;LA1=50;LA2=51;LA3=52;LA4=53;LSIO2=60;sm=10
        ...
        def rsz(sp,tp,edge,width, custom_sm=None, custom_entry=None):
            x1,y1=sp;x2,y2=tp;sm=custom_sm if custom_sm is not None else 20;pts=[(x1,y1)]
            entry = custom_entry if custom_entry is not None else active_entry_len
            if edge in['top','bottom']:
                sy=-1 if edge=='top'else 1;ys=y1+sy*sm;pts.append((x1,ys));ye=y2+sy*entry;ah=abs(ye-ys);nw=abs(x2-x1);sx=1 if x2>x1 else -1
                ...
                sx=1 if edge=='left'else -1;xs=x1+sx*sm;pts.append((xs,y1));xe=x2+sx*entry;aw=abs(xe-xs);nh=abs(y2-y1);sy=1 if y2>y1 else -1
        ...
        for p in all_pads:
            ...
            cell.add(gdstk.Label(lbl,(cx,cy),layer=LTXT,magnification=10,rotation=rot));poly_t,t_start=ct(p['rect'],edge,taper_length,trace_width);cell.add(poly_t);path_obj,path_pts=rsz(t_start,p['target'],edge,trace_width);cell.add(path_obj);p['trace_points']=path_pts
            
            # Inner leads from Region 1 to Region 2
            inner_path_obj_2, _ = rsz(p['target'], p['target_2'], edge, inner_trace_width, custom_sm=10, custom_entry=active_entry_len_2)
            cell.add(inner_path_obj_2)
            
            # Inner leads from Region 2 to Region 3
            inner_path_obj_3, _ = rsz(p['target_2'], p['target_3'], edge, inner_trace_width_3, custom_sm=5, custom_entry=active_entry_len_3)
            cell.add(inner_path_obj_3)
            
            # Inner leads from Region 3 to Region 4
            inner_path_obj_4, _ = rsz(p['target_3'], p['target_4'], edge, inner_trace_width_4, custom_sm=2, custom_entry=active_entry_len_4)
    ...
    def add_connection_wire(self, cell, label, width=0.040, layer=5):
        ...
        if label not in self.device_connection_points or label not in self.pad_connection_points:
            # print(f"Warning: Label '{label}' not found in both device and pad connection points.")
            return None
            
        p_pad = self.pad_connection_points[label]
        
        # Get device connection point (with translation)
        bbox = self.device_cell.bounding_box()
        ...
        # Create the wire and add to cell
    ...
    def visualize_and_save_pad_frame(self, lib, cell_to_show, title, gds_filename, show_plot=True):
        ...
        if show_plot:
            ...
            for p in polys:
                ...
                elif p.layer == 51: color, alpha, zorder = '#3498db', 0.3, 3 # Blue
                elif p.layer == 52: color, alpha, zorder = '#9b59b6', 0.3, 4 # Purple
                elif p.layer == 53: color, alpha, zorder = '#e74c3c', 0.3, 5 # Red
    ...
    def visualize_and_save_assembly(self, lib, cell_to_show, title, gds_filename, show_plot=True, zoom_factor=0.1):
        ...
        layer_config = {
            ...
            50: {'color': '#1abc9c', 'alpha': 0.4, 'label': 'Active 1 (L50)', 'zorder': 2},
            51: {'color': '#3498db', 'alpha': 0.4, 'label': 'Active 2 (L51)', 'zorder': 3},
            52: {'color': '#9b59b6', 'alpha': 0.4, 'label': 'Active 3 (L52)', 'zorder': 4},
            53: {'color': '#e74c3c', 'alpha': 0.4, 'label': 'Active 4 (L53)', 'zorder': 5},
        ...
        if self.active_center:
            ...
            # Use the same logic for zoom window
            ...
            ax2.set_xlim(acx - zw, acx + zw); ax2.set_ylim(acy - zh, acy + zh)
