...
class QuantumDotPadGenerator:
    def __init__(self):
        ...
        self.pad_params = {
            'N': 6, 'layout_width': 1400, 'layout_height': 1400,
            'pad_width': 100, 'pad_height': 100, 'pad_spacing': 30,
            'edge_margin': 50, 'active_size': 180, 'trace_width': 10,
    ...
    def _plot_gds(self, cell, title="量子点器件布局", show_plot=True):
        if not show_plot:
            return
    ...
    def _create_6qd_layout_with_labels(self, **kwargs):
        ...
        sg_top_thick = params['sg_top_thick']; sg_bot_thick = params['sg_bot_thick']; sg_extension = params['sg_extension']
        ...
        sg_top=gdstk.rectangle((sgxs,ysgtb),(sgxe,ysgtt),layer=LAYER_SG);ysgbt=ycb-gate_half_height-gap_gate_outer_sg;ysgbb=ysgbt-sg_bot_thick;
        sg_bot=gdstk.rectangle((sgxs,ysgbb),(sgxe,ysgbt),layer=LAYER_SG);all_shapes.extend([sg_top,sg_mid,sg_bot])
        lsg1=gdstk.Label("SG1",(sgxs,(ysgtb+ysgtt)/2),layer=LAYER_LABEL);cell.add(lsg1);
        connection_points["SG1"]=lsg1.origin;lsg2=gdstk.Label("SG2",(sgxs,(ysgmb+ysgmt)/2),layer=LAYER_LABEL);cell.add(lsg2);
        connection_points["SG2"]=lsg2.origin;lsg3=gdstk.Label("SG3",(sgxe,(ysgbb+ysgbt)/2),layer=LAYER_LABEL);cell.add(lsg3);
    ...
    def generate_quantum_dot_layout(self, **kwargs):
        self.device_lib, self.device_cell, self.device_connection_points = self._create_6qd_layout_with_labels(**kwargs)
