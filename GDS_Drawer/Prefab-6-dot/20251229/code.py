...
def create_rect_wire_layout(
    ...
    # ... (This function is kept as provided by the user, with the return fix)
...
class RouteManager:
    ...
    """
    def __init__(self, chip_center, active_region_bbox, quantum_dot_polygons, layer):
        ...
        self.pad_connection_extension_len = 10.0  # 500nm线连接pad引线的部分，垂直引出10um

        # 用于布线规划的参数 (可以根据需要调整)
        self.tier_radial_distances = {
            'tier1': 20.0, # 40nm线段的径向距离
            'tier2': 40.0, # 100nm线段的径向距离
            'tier3': 80.0  # 500nm线段的径向距离
        }
    ...
    def plan_and_generate_routes(self):
        ...
        for side, requests in requests_by_side.items():
            ...
            for i, req in enumerate(requests):
                ...
                # The transition point is the end of the 40nm segment and the start of the 100nm segment
                ...
                if len(full_path_points) > 1 and len(full_path_points) == len(full_path_widths):
                    ...
                    if merged_route_geo:
                        self.routes.append({'geo': merged_route_geo[0], 'label': label})
    ...
    def _calculate_target_waypoints(self, p0, p_final, side, index, total_on_side, electrode_direction):
        ...
        if electrode_direction == 'down':
            p0_ext[1] -= self.initial_extension_len
        elif side == 'top':
            p0_ext[1] += self.initial_extension_len
        elif side == 'bottom':
            p0_ext[1] -= self.initial_extension_len
        elif side == 'left':
            p0_ext[0] -= self.initial_extension_len
        elif side == 'right':
            p0_ext[0] += self.initial_extension_len
        waypoints['p0_ext'] = p0_ext

        # Tier 1 (40nm) waypoint
        radial_dist_tier1 = self.tier_radial_distances['tier1']
        lateral_offset_tier1 = (index - (total_on_side - 1) / 2.0) * (self.W['tier1'] + self.S['tier1'])
        
        p0_ext_to_p1 = np.array(p0_ext)
        if side == 'top':
            p0_ext_to_p1[1] = center_y + radial_dist_tier1
            p0_ext_to_p1[0] = center_x + lateral_offset_tier1
        elif side == 'bottom':
            p0_ext_to_p1[1] = center_y - radial_dist_tier1
            p0_ext_to_p1[0] = center_x + lateral_offset_tier1
        elif side == 'left':
            p0_ext_to_p1[0] = center_x - radial_dist_tier1
            p0_ext_to_p1[1] = center_y + lateral_offset_tier1
        elif side == 'right':
            p0_ext_to_p1[0] = center_x + radial_dist_tier1
            p0_ext_to_p1[1] = center_y + lateral_offset_tier1
        waypoints['p0_ext_to_p1'] = p0_ext_to_p1

        # Tier 2 (100nm) waypoint
        radial_dist_tier2 = self.tier_radial_distances['tier2']
        lateral_offset_tier2 = (index - (total_on_side - 1) / 2.0) * (self.W['tier2'] + self.S['tier2'])

        p1_to_p2 = np.array(p0_ext_to_p1)
        if side == 'top':
            p1_to_p2[1] = center_y + radial_dist_tier2
            p1_to_p2[0] = center_x + lateral_offset_tier2
        elif side == 'bottom':
            p1_to_p2[1] = center_y - radial_dist_tier2
            p1_to_p2[0] = center_x + lateral_offset_tier2
        elif side == 'left':
            p1_to_p2[0] = center_x - radial_dist_tier2
            p1_to_p2[1] = center_y + lateral_offset_tier2
        elif side == 'right':
            p1_to_p2[0] = center_x + radial_dist_tier2
            p1_to_p2[1] = center_y + lateral_offset_tier2
        waypoints['p1_to_p2'] = p1_to_p2

        # Tier 3 (500nm) waypoint
        radial_dist_tier3 = self.tier_radial_distances['tier3']
        lateral_offset_tier3 = (index - (total_on_side - 1) / 2.0) * (self.W['tier3'] + self.S['tier3'])

        p2_to_p3 = np.array(p1_to_p2)
        if side == 'top':
            p2_to_p3[1] = center_y + radial_dist_tier3
            p2_to_p3[0] = center_x + lateral_offset_tier3
        elif side == 'bottom':
            p2_to_p3[1] = center_y - radial_dist_tier3
            p2_to_p3[0] = center_x + lateral_offset_tier3
        elif side == 'left':
            p2_to_p3[0] = center_x - radial_dist_tier3
            p2_to_p3[1] = center_y + lateral_offset_tier3
        elif side == 'right':
            p2_to_p3[0] = center_x + radial_dist_tier3
            p2_to_p3[1] = center_y + lateral_offset_tier3
    ...
    def _create_orthogonal_or_45_path(self, p_start, p_end, side):
    ...
    def check_drc(self):
            ...
            if isinstance(geo, gdstk.Polygon):
                ...
                for k in range(len(vertices)):
                    ...
                    if not is_valid_angle:
                        print(f"[DRC角度违规] 路由 '{route_info['label']}' 发现一个角度为 {angle_deg:.2f}° 的非正交或45度转角。")
        ...
        print("进行间距检查...")
        # This is a simplified check. A full DRC would use boolean operations or grid-based checks.
        # For now, we'll check bounding boxes and assume proper routing ensures spacing.
        # A more robust check would involve gdstk.boolean('not') or similar.
        for i in range(num_routes):
            for j in range(i + 1, num_routes):
                route_i_geo = self.routes[i]['geo']
                route_j_geo = self.routes[j]['geo']

                # Calculate minimum required spacing based on widths
                width_i = self._get_route_max_width(route_i_geo)
                width_j = self._get_route_max_width(route_j_geo)
                min_spacing = max(width_i, width_j) # Spacing must be greater than line width

                # Use gdstk.boolean to find overlaps or near-overlaps
        ...
        for route_info in self.routes:
            ...
            if not gdstk.boolean([route_geo], [active_region_poly], 'and', layer=self.layer + 4):
                violations_found = True
    ...
    def _get_route_max_width(self, route_geo):
        # This is a heuristic to estimate the max width of a route polygon
        # For FlexPath, we know the widths, but for a general polygon, it's harder.
        # For simplicity, we'll assume the route polygons are somewhat rectangular
        # and take the smaller dimension of its bounding box as an estimate of width.
...
if __name__ == "__main__":
    ...
    print("--- 步骤 4 & 5: 正在使用全新 RouteManager 连接所有线... ---")
